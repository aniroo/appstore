...
def deploy(request):
    ...
    print("Request sent to tycho client start to start napari client app:")
    ...
    print(f"Service Napari: {services}")

    if status != 'success':
        raise Exception("Error encountered while starting Napari service: " + status)
    ...
    if ip_address == '' or ip_address == '--':
        raise Exception("napari ip_address is invalid: " + ip_address)

    if port == '' or port == '--':
        raise Exception("napari port is invalid: " + port)
